...
RUN apk --no-cache add ca-certificates wget curl tzdata vim openjdk8 git openssh-client curl unzip bash ttf-dejavu coreutils tini && \
    ...
    mkdir -p /usr/share/jenkins/ref/init.groovy.d && mkdir -p "$JENKINS_HOME" &&  mkdir -p /usr/share/jenkins/ref && \
...
#ENTRYPOINT ["/bin/ash" , "/usr/local/bin/jenkins.sh"]
#ENTRYPOINT ["/bin/ash"]
